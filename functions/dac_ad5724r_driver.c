...
void init_dac_ad5724r(volatile unsigned int * dac_addr, unsigned int dac_range, unsigned int DAC_CLR_msk) {
	...
	alt_write_word( ( dac_addr + SPI_TXDATA_offst ), WR_DAC | PWR_CNT_REG | DAC_A_PU | DAC_B_PU | DAC_C_PU | DAC_D_PU | REF_PU);   // power up reference voltage, dac A, dac B, dac C, and DAC D
	while (! ( alt_read_word(dac_addr + SPI_STATUS_offst) & ( 1 << status_TMT_bit ) ))
		;					// wait for the spi command to finish
	alt_write_word( ( dac_addr + SPI_TXDATA_offst ), WR_DAC | OUT_RANGE_SEL_REG | DAC_ALL | dac_range);   // set range to P50, PN50, etc etc
	while (! ( alt_read_word(dac_addr + SPI_STATUS_offst) & ( 1 << status_TMT_bit ) ))
		;					// wait for the spi command to finish
...
void wr_dac_ad5724r(volatile unsigned int * dac_addr, unsigned int dac_range, unsigned int dac_id, double volt, unsigned int DAC_LDAC_msk, uint8_t en_mesg) {
...
// read the value of the DAC, check warning, and redo the writing if necessary
	if (sdo_is_wired) {
		...
		usleep(100);
		print_warning_ad5724r(dac_addr, en_mesg);   // find out if warning has been detected

		// recursion to make sure it works
		if ( ( volt_int & 0x0FFF ) != ( dataread >> 4 )) {
			usleep(500000);
			wr_dac_ad5724r(dac_addr, dac_id, dac_range, volt, DAC_LDAC_msk, en_mesg);
		}
	}
