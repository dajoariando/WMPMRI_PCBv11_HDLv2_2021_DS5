...
void init_dac_ad5724r(volatile unsigned int * dac_addr, unsigned int DAC_CLR_msk) {
	...
	alt_write_word((dac_addr + SPI_TXDATA_offst), WR_DAC | OUT_RANGE_SEL_REG | DAC_ALL | PN50);   // set range voltage to +/- 5.0V
	while (!(alt_read_word(dac_addr + SPI_STATUS_offst) & (1 << status_TMT_bit)))
		;					// wait for the spi command to finish
...
void wr_dac_ad5724r(volatile unsigned int * dac_addr, unsigned int dac_id, double volt, unsigned int DAC_LDAC_msk, uint8_t en_mesg) {
	...
	if (sdo_is_wired) {
		...
		if ((volt_int & 0x0FFF) != (dataread >> 4)) {
			usleep(500000);
			wr_dac_ad5724r(dac_addr, dac_id, volt, DAC_LDAC_msk, en_mesg);
		}
	}
