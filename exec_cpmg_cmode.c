...
int main(int argc, char * argv[]) {
	...
	double p90_us = atof(argv[6]);// p90 pulse length.
	double p90_dchg_us = atof(argv[7]);// p90 discharging length.
	double p90_dtcl = atof(argv[8]);// p90 duty-cycle (unused at the moment and is overdriven by setting h2p_ph_overlap_addr).
	double p180_1st_pchg_us = atof(argv[9]);// p180 precharging by VPC.
	double p180_pchg_us = atof(argv[10]);// p180 precharging by VPC.
	double p180_us = atof(argv[11]);// p180 pulse length.
	double p180_dchg_us = atof(argv[12]);// p180 discharging length.
	double p180_dtcl = atof(argv[13]);// p180 duty-cycle (unused at the moment and is overdriven by setting h2p_ph_overlap_addr).
	double echoshift_us = atof(argv[14]);// shift the acquisition window.
	double echotime_us = atof(argv[15]);// echo spacing
	long unsigned scanspacing_us = atoi(argv[16]);// inter-experiment spacing
	unsigned int samples_per_echo = atoi(argv[17]);// samples per echo or number of points or SpE
	unsigned int echoes_per_scan = atoi(argv[18]);// echoes per scan or number of echoes
	unsigned int n_iterate = atoi(argv[19]);// number of iteration
	uint8_t ph_cycl_en = atoi(argv[20]);// set '1' to enable phase cycling and '0' to disable it
	unsigned int dconv_fact = atoi(argv[21]);// downconversion decimation factor (unused)
	unsigned int echoskip = atoi(argv[22]);// skip every n echoes (unused)
	unsigned int echodrop = atoi(argv[23]);// drop the first n echoes (unused)
	double vvarac = atof(argv[24]);// varactor voltage for the preamp (in V)
	// --- vpc precharging ---
	double lcs_vpc_pchg_us = atof(argv[25]);// precharging the lcs using VDD
	double lcs_recycledump_us = atof(argv[26]);// recycle the energy in lcs to VPC
	double lcs_vpc_pchg_repeat = atof(argv[27]);// repeat VPC precharging for n times
	// --- vpc discharging ---
	double lcs_vpc_dchg_us = atof(argv[28]);// discharging the VPC into lcs
	double lcs_wastedump_us = atof(argv[29]);// waste/dump the lcs energy into the protection diode
	double lcs_vpc_dchg_repeat = atof(argv[30]);// repeat VPC precharging for n times
	// --- gradient length and strength
	float gradz_volt = atof(argv[31]);// gradient z dac output voltage (can be either polarity, positive or negative)
	float gradx_volt = atof(argv[32]);// gradient x dac output voltage (can be either polarity, positive or negative)
	// enable lcs initial precharging and discharging
	char en_lcs_pchg = atoi(argv[33]);// enable the vpc precharging via lcs prior to cpmg
	char en_lcs_dchg = atoi(argv[34]);// enable the vpc discharging via lcs post cpmg
	unsigned int exp_num = atoi(argv[35]);// the experiment number
	...
	cpmg_cmode_obj cpmg_cmode_params;
	for (ii = 0; ii < n_iterate; ii++) {
		...
		cpmg_cmode_params = bstream__cpmg_cmode(
				f_larmor,
				larmor_clk_fact,
				adc_clk_fact,
				bstrap_pchg_us,
